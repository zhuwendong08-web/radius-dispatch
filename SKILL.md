...
> ⚠️ **高德 key 不共享**：本仓库不含任何高德 key，也读不到作者账号的 key。
> 想用「真实步行距离 / 等时圈 / 更全的中国场地 POI」等中国落地功能，
> **你必须自备高德开放平台 Web 服务 key**（环境变量 `AMAP_KEY` 或 `--key` 传入）。
> 不传 key 时自动降级走免 key 的 OSM，基础流程照样能跑——不会偷偷借用任何人的配额。
...
| 类型 | 场景 | 权重（交通/适配/面积/容量） | 默认硬约束 |
|---|---|---|---|
| `team-building` | 团建 / 活动场地，要大有容量、交通必须便利 | 0.3 / 0.5 / 0.1 / 0.1 | 交通 ≤1000m，目标 250 人 |
| `warehouse` | 仓储 / 物流用地，重面积与路网 | 0.2 / 0.1 / 0.6 / 0.1 | 交通 ≤5000m |
| `service-outlet` | 服务网点，重人流可达 | 0.5 / 0.3 / 0.1 / 0.1 | 交通 ≤800m |

**适配度维度**（suitability）是 2026-09-05 新增的第四维：高德不返回面积/容量，那两维恒为常数，
只有适配度能拉开差距——它用名称 + 类目信号判断「这像不像这类场地」。
...
3. **交通硬约束是可调的。** 默认 1000m（高德下按步行路网算，比直线严格）；真要严格，调到 500–800m。
4. **看 `flags` 列。** 「面积未知」「容量未知」「按类型推断」说明数据源没标，分数含推断/中性假设，需要人工核。

## 已知缺陷（重要）

1. **高德下交通距离为真实步行路网距离**（`/v3/direction/walking`，逐个候选算路）；
   OSM 数据源无路径规划，仍是直线距离。算路失败的候选回退直线并标「步行算路失败」。
   高德个人账号有 QPS 限制，长流程（200+ 请求）可能偶发失败——脚本已内置全局节流 + 退避重试。
...
2. 高德**不返回可容纳人数**，容量维走「容量未知」中性分。
   这两维在高德下会退化为中性分，**排名主要由「适配度」维度承担区分**（见上方权重表）。
交通硬约束（真实步行距离）负责一票否决不便利的，适配度负责排出哪些场地更对口。
